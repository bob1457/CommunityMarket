...
using AppUserProfile = CommonMarket.Web.Models.AppUserProfile;
...
namespace CommonMarket.Web.Controllers
{
    ...
    public class AccountController : BaseController
    {
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<JsonResult> SignIn( string Email, string Password, bool RememberMe, bool lockout) //Accept ajax request to login
        {
            var result = await SignInManager.PasswordSignInAsync(Email, Password, RememberMe, lockout);
            //ApplicationUser user = UserManager.FindAsync(Email, Password);
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (model.Supplier)
                    {
                        message = "New user: " + model.Email +
                                         " just registered as supplier. Setup may be required.";
                    }
                    else
                    {
                        message = "New user: " + model.Email + " just registered.";
                    }
        ...
        #region Test using service layer

        public ActionResult CreateUserEntity()
        {
            return View();
        }

        //[HttpPost]
        //public ActionResult CreateUserEntity(Customer customer)
        //{
        //    if (ModelState.IsValid)
        //    {
        //        //Get user input data
        ...
        //Ajax reset password (forgot password?)
        [HttpPost]
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<JsonResult> PasswordReset(string Email)
        {
            if (ModelState.IsValid)
            {
                ...
                if (user == null || !(await UserManager.IsEmailConfirmedAsync(user.Id)))
                {
                    // Don't reveal that the user does not exist or is not confirmed
                    //return View("ForgotPasswordConfirmation");
                    return Json("Request has been sent, check your email for details!");
                }
        ...
        //[AllowAnonymous]
        //[ValidateAntiForgeryToken]
        //public JsonResult PasswordReset(string Email)
        //{
        //    if (ModelState.IsValid)
        //    {
        ...
        //        UserManager.SendEmail(Id, "Reset Password", "Please reset your password by clicking here: <a href=\"" + callbackUrl + "\">link</a>");
        //        //ViewBag.Link = callbackUrl;
        //        //return View("ForgotPasswordConfirmation");
        //        return Json("Request has been sent, check your email for details!");
        //    }
